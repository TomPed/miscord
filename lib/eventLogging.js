...
function history(event, bot, channel, user) {
  if (event === 'leave' && user.bot === false) {
    bot.sendMessage(channel.server.get('name', 'history'), '**' + user.username + '**' + ' left **' + channel.name + '** at **' + dateFormat(new Date(), 'mmmm dS, yyyy, h:MM:ss TT Z') + '**');
    bot.sendMessage(channel.server.get('name', 'history'), ' ');
  }

  if (event === 'join' && user.bot === false) {
    bot.sendMessage(channel.server.get('name', 'history'), '**' + user.username + '**' + ' joined **' + channel.name + '** at **' + dateFormat(new Date(), 'mmmm dS, yyyy, h:MM:ss TT Z') + '**');
    bot.sendMessage(channel.server.get('name', 'history'), ' ');
}
}
